      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-700 ${
        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
          <div className="flex justify-between items-center py-4 lg:py-6">
            <Link to="/" className="text-xl lg:text-2xl xl:text-3xl font-serif font-light tracking-[0.15em] text-dark-gray hover:text-energetic-blue transition-colors duration-500">
              SOPHIE WAGNER
            </Link>
            ...
            <div className="hidden xl:flex items-center space-x-6 lg:space-x-8">
              {navItems.map((item) => (
                <Link
                  key={item.name}
                  to={item.path}
                  className={`text-dark-gray/70 hover:text-energetic-blue transition-all duration-500 font-light tracking-[0.05em] text-sm relative group whitespace-nowrap ${
                    location.pathname === item.path ? 'text-energetic-blue' : ''
                  }`}
                >
                  {item.name}
                  <span className={`absolute -bottom-1 left-0 h-0.5 bg-energetic-blue transition-all duration-500 ${
              ...
              <Link
                to="/kontakt"
                className="bg-energetic-blue text-fresh-white px-6 lg:px-8 py-3 lg:py-3.5 rounded-full hover:bg-deep-blue hover:shadow-warm-lg transition-all duration-500 font-medium tracking-[0.05em] text-sm transform hover:scale-105 whitespace-nowrap"
              >
                Book konsultation
              </Link>
            </div>
            ...
            {/* Mobile Menu Button */}
            <button
              className="xl:hidden p-3 text-dark-gray hover:bg-energetic-blue/10 rounded-full transition-all duration-300 relative z-50"
